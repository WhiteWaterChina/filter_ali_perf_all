...
def get_data():
    work_book = xlsxwriter.Workbook("阿里多盘性能数据.xlsx".decode('gbk'))
    sheet_now = work_book.add_worksheet("多盘读写性能数据".decode('gbk'))
...
#                bw, latency, qos_data = filter_1job(temp_text)
                bw = filter_1job(temp_text)
            else:
#                bw, latency, qos_data = filter_4job(temp_text)
                bw = filter_4job(temp_text)
            data_bw.append(bw)
...
#                iops, latency, qos_data = filter_1job_random(temp_text)
                iops = filter_1job_random(temp_text)
            else:
#                iops, latency, qos_data = filter_4job_random(temp_text)
                iops = filter_4job_random(temp_text)
...
#            data_qos_random.append(qos_data)
    ...
    if len(var_char_entry_mix_result.get()) != 0:
        ...
        for index_policy_mix, item_policy_mix_sub in enumerate(policyListJobmix):
            ...
            if if_4jobs is None:
#                bw, latency, qos_data = filter_1job_mix(temp_text)
                iops = filter_1job_mix(temp_text)
            else:
#                bw, latency, qos_data = filter_4job_mix(temp_text)
                iops = filter_4job_mix(temp_text)
...
#            data_qos_mix.append(qos_data)
    ...
    tkMessageBox.showinfo("完成".decode('gbk'), "生成的结果在《阿里多盘性能数据.xlsx》中，请自行查看！".decode('gbk'))
    work_book.close()


root = Tkinter.Tk()
root.title("阿里性能测试多盘(dual)结果处理工具".decode('gbk'))
...
Tkinter.Label(frame_top_top, text='请在如下选择需要处理的阿里多盘性能测试顺序读写的结果文件(Seq)'.decode('gbk'), bg='Red').pack()
...
Tkinter.Label(frame_middle_top, text='请在如下选择需要处理的阿里多盘性能测试随机读写的结果文件(Random)'.decode('gbk'), bg='Red').pack()
...
Tkinter.Label(frame_bottom_top, text='请在如下选择需要处理的阿里多盘性能测试混合读写的结果文件(Mix)'.decode('gbk'), bg='Red').pack()
